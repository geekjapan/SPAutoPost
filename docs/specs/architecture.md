...
- Graph Hosted Runtime: user-assigned managed identity（application permission）を第一候補に決定（Issue #2）。#27 で正式確定を継続する
...
- Identity: #27 Graph 認証方式の正式確定（Issue #2 で managed identity を第一候補決定済み、#27 で本番確定） / #29 Admin API/UI 向け Entra ID login /
  #32 local Graph delegated posting PoC

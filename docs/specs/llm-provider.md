...
Proposed (Spike 評価完了 — 2026-06-26 更新)
...
| `test_manual` | optional（手動取込のみ） | UI 自動化・API 利用禁止 |
...
```text
Provider.validateConfig(config: ProviderConfig) -> ProviderStatus
Provider.generateDraft(input: DraftInput) -> DraftOutput
Provider.getProviderMetadata() -> ProviderMetadata
Provider.estimateCost(input: DraftInput) -> CostEstimate | None  # optional
```
...
{
  is_valid: bool,
  error_code: str | None,   # provider_config_invalid, provider_auth_failed 等

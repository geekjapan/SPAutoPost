...
各文書の M0 における位置づけ（Accepted / finalization-tracked / M1+ defer）と未決事項の集約先（#2 / #15 等）の正本は `docs/design-documents.md` の **Review & Status Matrix (Issue #23)** です。下表の Status と矛盾する場合はマトリクスを優先します。
...
| `data-model.md` | Accepted for M0 | #3, #10, #20, #28 | Advisory / DraftPost / Publication / AuditEvent |
...
| `audit-log.md` | Approved | #5, #10, #19, #22, #29 | 監査ログ、correlation ID、保存禁止項目、user principal 記録 |
| `security-baseline.md` | Approved | #5, #15, #22, #27, #29 | Secret、権限最小化、LLM 入力制限、投稿安全性 |
| `configuration.md` | Accepted for M0 | #4, #6, #9, #10 | config、環境変数、feature flag、dry-run |

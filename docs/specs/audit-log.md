...
M6 Production Hardening では、Azure Container Apps / Jobs の console logs と
system logs を Log Analytics workspace に送信し、`AuditEvent` JSON log line を
`correlation_id`、`error_code`、publication result で検索できるようにします。

Log Analytics に送信してよい項目:
...
- created_at
- operation
- draft_id / publication_id / related_ids
- idempotency_key
- error_code / redacted error_message
- target_site_id / sharepoint_page_id（Secret ではない識別子のみ）

Log Analytics に送信してはいけない項目:

- access token / refresh token
- client secret / certificate private key
- cookie / authorization header
- raw prompt に含まれる機微情報
- 不要な個人情報

運用確認手順と KQL snippets は `docs/runbooks/log-analytics.md` と
`deploy/log-analytics.queries.kql` を参照します。workspace ID、alert、dashboard は
tenant 固有値のため repository に保存しません。

## Failure Audit

失敗時に記録する項目（AuditEvent の定義フィールド）:

- error_code
- error_message
- correlation_id
- created_at
- related_ids（retryable / failure_count / target など追加コンテキストを格納）
- operation（publication 失敗時のみ。`dry-run` / `create` / `update` / `publish` のいずれか）

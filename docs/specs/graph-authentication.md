...
この Spec は、SPAutoPost が Microsoft Graph を通じて SharePoint Site Page / News article を作成、更新、下書き、公開するための認証・認可方式を比較し、MVP で採用する方式を決めるための論点を整理します。

## Context

SPAutoPost の運用コアは Azure Container Apps / Azure Container Apps Jobs に置きます。

そのため、SharePoint 投稿処理は人間ユーザーの端末ではなく、Azure 上の実行環境から Microsoft Graph を呼び出します。
...
ローカル PoC は delegated、Azure hosted runtime は application permission または managed identity を使う方式です。
...
1. ローカル PoC / 初期疎通: delegated permission を許容
2. Azure hosted core / scheduled jobs: app-only access または managed identity を本命にする
3. 本番方向: managed identity + site/page library scope の最小権限化を第一候補として検証する

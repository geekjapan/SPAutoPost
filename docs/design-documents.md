...
| `docs/specs/graph-authentication.md` | M0 — finalization tracked | M0 | #2, #9, #24, #27, #32 | **#27**（Graph auth model: delegated / application / managed identity）, **#2** |
...
- M0 で未確定の spec gap: **#2**（SharePoint contract）、**#5**（security / secrets / audit / compliance baseline）、**#27**（Microsoft Graph authentication model）。いずれも M0 Milestone・OPEN。
...
- M0 の残り spec 確定（#2 / #5 / #27）が完了した時点で、`security-baseline.md` / `audit-log.md` を「M0 — finalization tracked」から Accepted へ昇格する。

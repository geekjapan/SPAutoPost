...
| `docs/specs/graph-authentication.md` | Accepted for M0（#27 確定済み） | #2, #9, #24, #32 | local PoC delegated（Device Code Flow）、hosted runtime user-assigned managed identity 確定、Graph permission 最小化・対象 site 固定方針 |
...
| `docs/specs/llm-provider.md` | Accepted for M1 scope | #6, #15, #16, #17, #18, #35 | LLM provider 分類、test_manual、Foundry / Azure OpenAI provider 方針（#35 で M1 mock 必須・API M3 以降確定） |
...
| `docs/specs/audit-log.md` | Approved | #5, #10, #19, #22, #29, #36 | 監査ログ、correlation ID、保存禁止項目、user principal 記録 |
| `docs/specs/security-baseline.md` | Approved | #5, #15, #22, #27, #29 | Secret、権限最小化、LLM 入力制限、投稿安全性 |
...
| `docs/decisions/2026-06-22-llm-provider-strategy.md` | Accepted | 実稼働 provider と test provider の分離判断（#35 で Accepted に昇格） |
...
| `docs/runbooks/production-hardening.md` | Draft | M6 本番投入前 hardening、observability、audit review、security review checklist |
...
- Approved: Issue クローズを経て内容が正式確定済み。Review & Status Matrix の **Accepted for M0** disposition bucket に属する。実装はこれに従う。
- Deprecated: 廃止予定。
...
| `docs/specs/security-baseline.md` | Accepted for M0（#5 closed） | M0 | #5 | **#15**（LLM 入力制限）, #22 |
| `docs/specs/audit-log.md` | Accepted for M0（#5 closed） | M0 | #5, #10, #19, #22 | #22（retention 本番確定） |
| `docs/specs/graph-authentication.md` | Accepted for M0（#27 確定済み） | M0 | #2, #9, #24, #32 | **#2**（Graph permission 付与確認・News promote 等は M1 以降） |
| `docs/specs/llm-provider.md` | Accepted for M1 scope (#35) | M1 (mock) / M3 (API providers) | #6, #15, #16, #17, #18, #35 | **#15**（provider 契約・費用・prompt/output 保存・切替方針）, **#16**（Azure OpenAI M3 前提条件）, **#17**（generic API M3 前提条件） |
| `docs/decisions/2026-06-22-llm-provider-strategy.md` | Accepted (#35) | M1 | #6, #15, #16, #17, #18, #35 | — |
...
| `docs/runbooks/production-hardening.md` | Draft runbook → M6 | M6 | #22 | #22（retry/backoff、Secret 混入、Graph/LLM review、observability、audit log review、security checklist） |
| `docs/runbooks/security-review.md` | Draft runbook → M6 | M6 | #5, #22 | #22 |
| `docs/runbooks/incident-response.md` | Draft runbook → M6 | M6 | #22 | #22 |

### 未決事項の集約（routing）

- **SharePoint お知らせ掲示板 contract** の未決事項（Graph permission 付与確認・必要最小権限の M1 検証、公開範囲、添付・画像、News publish/promote、投稿失敗時挙動）は **Issue #2** に集約する。Graph 認証方式（delegated / managed identity / app-only）は #27 で確定済みのため #2 では再検討しない。MVP の投稿方式は Site Page / News として Accepted 済みであり、この review では List item vs Site Page/News の選択を再オープンしない。参照元: `sharepoint-publishing.md`「Open Questions」、sharepoint ADR「Open Follow-ups」。本レビューは #2 の残 contract 判断自体は行わない（route のみ）。
- **LLM provider 戦略** の未決事項（production_api / production_flow / generic_api / test_manual の分離、provider 契約・費用、prompt/output 保存方針、provider 切替方針）は **Issue #15** に集約する。参照元: `llm-provider.md`、`security-baseline.md`、llm-provider-strategy ADR。本レビューは #15 の戦略判断自体は行わない（route のみ）。**#35 で M1 スコープ確定（mock 必須・API M3 以降）、ADR Accepted 済み。残残は #16（Azure OpenAI M3 前提条件）・#17（generic API M3 前提条件）で追跡。**

### 実装前 Spec 不足の追跡（Issue 化）

実装前に確定が必要な spec 不足は、すべて既存 Issue で追跡済み。**本レビューでは新規の投機的 Issue を作成しない。**

- M0 で未確定の spec gap: **#2**（SharePoint contract）のみ。~~**#5**（security / secrets / audit / compliance baseline）~~（#5 Closed・Approved）、~~**#27**（Microsoft Graph authentication model）~~（#27 確定済み）。
...
- ~~`security-baseline.md` / `audit-log.md` を「M0 — finalization tracked」から Accepted へ昇格する。~~ → **完了（#5 Closed、Approved に昇格済み）**。
- ~~M0 の残り spec 確定（#2 / #27）が完了した時点で、`graph-authentication.md` を「M0 — finalization tracked」から Accepted へ昇格する。~~ → **完了（#27 確定済み）**。

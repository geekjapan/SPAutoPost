...
- M0 で未確定の spec gap: **#2**（SharePoint contract）、~~**#5**（security / secrets / audit / compliance baseline）~~（#5 Closed・Approved）、**#27**（Microsoft Graph authentication model）。
...
- M0 の残り spec 確定（#2 / #27）が完了した時点で、`graph-authentication.md` を「M0 — finalization tracked」から Accepted へ昇格する。

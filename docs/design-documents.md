...
| `docs/specs/sharepoint-publishing.md` | Accepted for M0（MVP publishing mode） | M0 / M1 | #2, #9, #20, #36 | **#2**（List/Page・権限・公開範囲・添付・News promote） |
...
- **SharePoint 投稿方式・お知らせ掲示板 contract** の未決事項（List item か Site Page/News か、必要 Graph 権限、delegated / application / managed identity、公開範囲、添付・画像、News publish/promote、投稿失敗時挙動）は **Issue #2** に集約する。参照元: `sharepoint-publishing.md`「Open Questions」、sharepoint ADR「Open Follow-ups」、`graph-authentication.md`。本レビューは #2 の contract 判断自体は行わない（route のみ）。

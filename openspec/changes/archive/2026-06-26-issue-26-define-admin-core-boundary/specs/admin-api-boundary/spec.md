...
## Test Anchors（#31 / #28 流用観点）

各 scenario は #31（Admin API skeleton）・#28（storage baseline）のテストケースとして直接流用できる。
対応表:

| Scenario | 対象 Issue | テスト層 |
|---|---|---|
| DraftPost 一覧の取得 | #31 | Admin API integration |
| approve 要求の受付 → AdminCommand 1 件 insert・accepted/pending 返却 | #31, #28 | Admin API + storage |
| 重複要求の idempotent 処理 | #31, #28 | Admin API + storage |
| command 消費による状態遷移（Python worker） | #28 | Python unit/integration |
| 不正遷移の拒否（failed + error_code） | #28 | Python unit |
| pending から結果表示への遷移 | #31 | Admin API integration |
| edit の楽観反映 | #31, #28 | Admin API + storage |
| client key が無い状態変更要求を拒否する | #31 | Admin API unit |
| client key による retry 重複吸収 | #31 | Admin API unit |

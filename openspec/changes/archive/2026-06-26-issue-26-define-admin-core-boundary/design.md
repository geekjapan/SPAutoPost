...
- **idempotency**: Admin API は state-changing request に client 供給の `Idempotency-Key` を必須とし、route / draft / command type と client key から AdminCommand `idempotency_key` を導出して重複要求を吸収する。
...
- **client Idempotency-Key は必須**とする（grill R2 で確定）。Admin API は `Idempotency-Key` ヘッダが無い状態変更要求を拒否しなければならない（MUST NOT accept）。route + draft + command type + client key から server 側で `idempotency_key` を導出してよい（MAY）。

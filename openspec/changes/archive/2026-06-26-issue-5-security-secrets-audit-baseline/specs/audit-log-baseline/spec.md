...
システムは、失敗イベントに `error_code`・`error_message`・`correlation_id`・`created_at` を記録しなければならない（SHALL）。`retryable`・`failure_count`・`target` など追加コンテキストは `related_ids` に格納する。投稿（publication）失敗イベントは `operation` も記録する（値は `dry-run` / `create` / `update` / `publish` のいずれか）。

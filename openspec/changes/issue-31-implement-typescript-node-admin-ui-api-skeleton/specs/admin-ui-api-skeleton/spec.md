...
- **WHEN** 管理者 principal が無い状態で `GET /api/drafts` を要求する
- **THEN** Admin API は DraftPost data を返さず authentication error を返す
...
#### Scenario: secret payload key を拒否する
- **WHEN** request body に `access_token` / `cookie` / `authorization` など secret 相当 key が含まれる
...
- **THEN** Admin API は command status と error_code / error_message を返し、payload / idempotency_key は返さない

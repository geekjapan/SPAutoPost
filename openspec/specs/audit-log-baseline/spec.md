...
システムは、失敗イベントに `operation`・`error_code`・`retryable`・`failure_count`・`target`・`correlation_id`・`timestamp` を記録しなければならない（SHALL）。

#### Scenario: 投稿失敗の記録
- **WHEN** SharePoint への投稿が失敗する
- **THEN** error_code・retryable・failure_count を含む失敗監査ログが記録される

### Requirement: レビュー・承認の監査情報を記録する
システムは、review / approve / reject イベントに `draft_id`・`reviewer`・`action`・`previous_status`・`next_status`・`timestamp` を記録しなければならない（SHALL）。

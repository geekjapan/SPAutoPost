...
3. `published` → 下書き page の作成完了（SharePoint 上では未公開。`published` は Publication 処理の完了を意味し SharePoint 上の公開状態ではない）
4. `dry_run` → dry-run モードで実行（作成・更新なし）
5. `skipped` → 冪等性チェックにより作成スキップ
6. `failed` → エラー発生

M1 では page の更新（update）・削除（delete）操作は対象外とする。update/delete の挙動は後続 Issue で定義する。
...
### Requirement: カテゴリ・優先度フィールドおよびリンクの扱いを定義する

M1 では SharePoint 投稿にカテゴリ（taxonomy/metadata）・優先度（priority）フィールドを設定してはならない（SHALL NOT）。本文テキスト中のインライン URL は M1 スコープ内とし、テキストとして本文に含めて良い。

#### Scenario: カテゴリフィールドは M1 非対応として拒否される
- **WHEN** DraftPost にカテゴリまたはタクソノミーメタデータが含まれている
- **THEN** バリデーションエラーが返され、投稿は実行されない（M1 非対応として明示）

#### Scenario: 優先度フィールドは M1 非対応として拒否される
- **WHEN** DraftPost に優先度フィールドが含まれている
- **THEN** バリデーションエラーが返され、投稿は実行されない（M1 非対応として明示）

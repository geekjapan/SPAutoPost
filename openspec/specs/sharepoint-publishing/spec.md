...
M1 では SPAutoPost は SharePoint Site Page を下書き（`draft`）状態で作成しなければならない（SHALL）。SPAutoPost から `publish` エンドポイント（`/pages/{id}/microsoft.graph.sitePage/publish`）を呼ぶことは M1 では禁止する（SHALL NOT）。
...
- `duplicate_detected` → 非 retryable（idempotency_key 一致）
- `publish_rejected` → 非 retryable

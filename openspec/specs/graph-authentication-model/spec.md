...
Microsoft Graph への認証方式を定義する。local PoC（開発者端末）と Azure hosted runtime（本番）で使用する認証フロー・identity・permission を規定し、Secret 漏洩リスクと permission 過剰付与を防止する。
...
SPAutoPost に付与する Microsoft Graph permission は、SharePoint Site Page / News 投稿に必要な最小セットに限定しなければならない（SHALL）。`Sites.Selected` permission を優先して使用し、`Sites.ReadWrite.All` 等の広範な permission は原則避けなければならない（SHALL）。ただし、M1 検証で `Pages.ReadWrite.All` が site page 作成に対応していないことが確認された場合に限り、`Sites.ReadWrite.All` を例外的に採用し decision record に例外理由を記録してよい（MAY）。
...
#### Scenario: 広範 permission のレビュー
- **WHEN** Graph permission セットをレビューする
- **THEN** `Sites.ReadWrite.All`、`Sites.FullControl.All` 等の tenant 全体に影響する permission が付与されている場合は、decision record に例外理由が記録されていることを確認できる

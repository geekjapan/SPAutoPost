# Firecrawl Source Adapter Specification

## Purpose

Firecrawl API を使った URL → Markdown 取得の source adapter。
構造化 API のない vendor advisory / ブログ記事 / 公開セキュリティ情報を取り込む用途。
M1 optional（spike 評価済み）、本番採用は M2 以降に判断する。

参照: `docs/spikes/firecrawl-adapter-spike.md`

-- SPAutoPost storage baseline (PostgreSQL = schema 正本)
-- canonical entity (SourceRecord, Advisory, DraftPost, ReviewEvent, Publication,
-- AuditEvent) + AdminCommand queue。
-- 規約: enum は TEXT + CHECK（native ENUM 不使用）、配列/ネストは JSONB、
-- timestamp は timestamptz。queryable な列のみ column 化し index。
...
-- Admin UI/API ↔ Python core 非同期境界（ADR admin-core-boundary）の inbox。
